...
class SprintList extends React.Component {
  ...
  componentDidMount() {
    const fetchedTickets = tickets.filter(
      (ticket) => ticket.sprintId === this.props.sprint._id
    )
    ...
    return (
      <ul className="sprint">
        <li className="sprint__header">
          <h3 className="sprint__title">
            {this.props.backlog ? 'Backlog' : this.props.sprint.name}
          </h3>
        </li>
